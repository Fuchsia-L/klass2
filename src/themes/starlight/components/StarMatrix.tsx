...
import { StarlightMoon, StarlightSheetBtn, type StarlightTimelineEvent } from './parts';
...
export function StarMatrix({ p, events, weekStart, semesterWeek, weekOffset, onPrevWeek, onNextWeek, onResetWeek, onOpenEvent }: Props) {
  const normalizedWeekStart = dayStart(weekStart);
  const now = new Date();
  ...
  const blocks = events.map((event) => blockForEvent(event, normalizedWeekStart)).filter((block): block is MatrixBlockData => !!block);

  return (
    <View testID="starlight-matrix" style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerText}>
          <Text style={[styles.kicker, { color: p.subtle }]}>Week {semesterWeek ?? '-'} · {formatRange(normalizedWeekStart)}</Text>
          <Text style={[styles.title, { color: p.ink }]}>Constellation</Text>
        </View>
        <View style={styles.nav}>
          {weekOffset !== 0 ? <StarlightSheetBtn p={p} label="This week" onPress={onResetWeek} testID="starlight-matrix-reset-week" /> : null}
      ...
      <View style={[styles.dayStrip, { borderBottomColor: p.line }, WEB_DAY_STRIP_STYLE]}>
        ...
        {DAYS.map((day, index) => {
          ...
          return (
            ...
            >
              <Text style={[styles.dayName, { color: active ? (p.dark ? p.bg : '#ffffff') : p.ink }]}>{day}</Text>
              <Text style={[styles.dayNum, { color: active ? (p.dark ? p.bg : '#ffffff') : p.ink }]}>{dayNums[index]}</Text>
            </View>
          );
        })}
      </View>
        ...
        <View style={[styles.gridInner, { height: HOUR_HEIGHT * HOURS.length + 10 }]}>
          <View style={[styles.hourGutter, { borderRightColor: p.line }]}>
            {HOURS.map((hour, index) => (
              <Text key={hour} style={[styles.hourText, { top: index * HOUR_HEIGHT - 7, color: p.subtle }]}>
          ...
          <View style={styles.columns}>
            {HOURS.map((_, index) => (
              <View key={index} pointerEvents="none" style={[styles.hourLine, { top: index * HOUR_HEIGHT, borderTopColor: p.line }]} />
            ))}
            {DAYS.map((day, index) => (
              <View key={day} style={[styles.column, { borderRightColor: p.line, borderRightWidth: index < 6 ? 1 : 0 }]}>
                {blocks
                  ...
                  .map((block) => (
                    <MatrixBlock key={`${block.event.id}-${block.event.start_time}`} p={p} block={block} onPress={() => onOpenEvent(block.event.id)} />
                  ))}
              </View>
            ))}
            {showToday ? (
              <View pointerEvents="none" testID="starlight-matrix-now-marker" style={[styles.nowLine, { top: nowTop, backgroundColor: p.nowLine, shadowColor: p.nowGlow }]}>
...
function MatrixBlock({ p, block, onPress }: { p: StarlightPaletteColors; block: MatrixBlockData; onPress: () => void }) {
    ...
    <Pressable
      ...
      style={({ pressed }) => [
        styles.block,
        {
          top: block.top,
          height: block.height,
          backgroundColor: isNext ? p.accent : rgbaFromHex(categoryColor, p.dark ? 0.2 : 0.14),
          borderColor: isNext ? 'transparent' : rgbaFromHex(categoryColor, 0.35),
          borderLeftColor: isNext ? p.accent2 : categoryColor ?? p.line,
    ...
    >
      <Text
        style={[
          styles.blockTitle,
          {
            color: isNext ? (p.dark ? p.bg : '#ffffff') : isPast ? p.dim : p.ink,
      ...
      {!thin && block.event.location ? (
        <Text style={[styles.blockMeta, { color: isNext ? (p.dark ? p.bg : '#ffffff') : p.subtle, opacity: isNext ? 0.74 : 1 }]} numberOfLines={1}>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  header: { paddingTop: 32, paddingHorizontal: 22, paddingBottom: 14, flexDirection: 'row', alignItems: 'flex-end', gap: 12 },
  headerText: { flex: 1, minWidth: 0 },
  kicker: { fontFamily: 'Inter-Medium', fontSize: 10, letterSpacing: 3, fontWeight: '500', textTransform: 'uppercase' },
  title: { marginTop: 6, fontFamily: 'Fraunces-Regular', fontSize: 36, lineHeight: 40, fontWeight: '400', fontStyle: 'italic' },
  nav: { flexDirection: 'row', alignItems: 'center', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end', maxWidth: 156 },
  ...
  dayNum: { marginTop: 1, fontFamily: 'Fraunces-Medium', fontSize: 15, fontWeight: '500', fontVariant: ['tabular-nums'] },
  gridScroll: { flex: 1 },
  ...
  hourLine: { position: 'absolute', left: 0, right: 0, borderTopWidth: 1, borderStyle: 'dashed', opacity: 0.7 },
  ...
  blockTitle: { fontSize: 10, lineHeight: 13, fontWeight: '500' },
  blockMeta: { marginTop: 2, fontSize: 9, lineHeight: 11, fontWeight: '500' },
  nowLine: { position: 'absolute', left: 0, right: 0, height: 1, zIndex: 3, shadowOpacity: 0.9, shadowRadius: 8, shadowOffset: { width: 0, height: 0 } },
  ...
  emptyText: { marginTop: 4, fontSize: 12, lineHeight: 18 },
});
