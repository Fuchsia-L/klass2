...
- Today/Week/Todos/Settings rendering is delegated to `StarHome`/`StarMatrix`/`StarTodos`/`StarSettings` (see below). `StarTodoSheet` + `StarEventSheet` are real create/edit sheets; `StarlightRatingSheet` remains a Phase-3 `SheetFrame` placeholder. All screens accept palette colors + plain data props — feature-hook calls live only in `StarlightRoot` and `StarTodoSheet` (todo service).
