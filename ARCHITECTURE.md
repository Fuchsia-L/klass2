...
- `components/StarlightRoot.tsx` — Container. Wires `useTheme`/`useThemeSettings`/`useEvents`/`useSemesterConfig`/`useRatings`/`useTodos` into palette-driven Starlight screens + modal sheets. Delegates Today/Week/Todos/Settings rendering to `StarHome`/`StarMatrix`/`StarTodos`/`StarSettings`; mounts `StarTodoSheet` for create/edit plus placeholder `StarlightEventSheet`/`StarlightRatingSheet` (SheetFrame-based). Holds tab/event-sheet/rating-sheet/todo-sheet/week-offset/nudge-dismissal state. `TodoSheetState` union is `{mode:'create'} | {mode:'edit', todo} | null`. Internal `Screen` wrapper cross-fades screens with RN `Animated.timing`. Module-local helpers `withState(events, nowMs)`, `latestRatingsByEventId(ratings)`, `formatTime`, `nudgeCopy`, `routeToTab`, `sameDay`.
...
- Today/Week/Todos/Settings rendering is delegated to `StarHome`/`StarMatrix`/`StarTodos`/`StarSettings` (see below). `StarTodoSheet` is a real create/edit sheet; `StarlightEventSheet`/`StarlightRatingSheet` remain Phase-3 `SheetFrame` placeholders. All screens accept palette colors + plain data props — feature-hook calls live only in `StarlightRoot` and `StarTodoSheet` (todo service).
- Add-button dispatch (`StarlightTabBar.onAdd`): sets `todoSheetState={mode:'create'}` when `tab==='todos'`, otherwise opens `StarlightEventSheet` in create mode (`eventSheetId='new'`).
...
### StarSettings (`src/themes/starlight/components/StarSettings.tsx`)
