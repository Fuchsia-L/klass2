...
- `src/themes/minimal/components/MinRatingSheet.tsx` — minimal rating input bottom sheet (stars + efficiency + mood + reflection). B3 fix: seed effect depends only on `[open, event?.id]`, so mid-edit local state survives ratings-list refreshes for the same open event.
- `src/themes/minimal/components/MinRatingSheet.test.tsx` — minimal rating sheet tests: edit stability (B3) — local draft survives refreshed `existing` prop for the same event; reseed on event id change.
...
- `src/themes/minimal/components/parts/MinSheetBtn.tsx` — minimal sheet button (`primary` / default variants). Primary paints `backgroundColor: p.ink` with `color: p.bg`, non-primary keeps transparent fill with `p.ink` text; disabled state uses `p.panel` / `p.dim`. Currently uses `marginLeft: -1` on the primary variant for the edge-to-edge stroke join (H1 polish pending). Callers: `MinEventSheet`, `MinRatingSheet`.
...
- `MinHome`, `MinMatrix`, `MinTodos`, `MinSettings`, `MinEventSheet` — tab bodies / sheets consumed by `MinimalRoot`. `MinSettings` exposes `onChangeTheme` which, per B1, switches to the most recently used non-minimal palette (or falls back to `legacyPackage.palettes[0].id`).
- `MinRatingSheet({ p: MinimalPaletteColors, event: MinimalEvent | null, existing?: TimeSlotRating, onClose: () => void, onSave: (payload: { efficiency: 1|2|3|4|5; moodIndex: 1|2|3|4|5; reflection: string }) => void })` — minimal rating input sheet. Open state is derived from `event !== null`. **B3 invariant**: the seeding `useEffect` depends only on `[open, event?.id]`, so refreshed `existing` values (e.g. after a ratings list refresh for the same open event) do NOT clobber mid-edit local `efficiency` / `mood` / `reflection` state; reseeding happens only on open transition or event-id change. Exposes stable test hooks: `testID="min-rating-reflection"` (TextInput), `testID="min-rating-efficiency-{1..5}"` (efficiency bars), `testID="min-rating-mood-{1..5}"` (mood cells). Caller: `MinimalRoot`.
...
- `MinSheetBtn({ p: MinimalPaletteColors, label: string, onPress: () => void, primary?: boolean, flex?: number, disabled?: boolean })` — bottom-sheet action button. Primary renders with `backgroundColor: p.ink` / `color: p.bg`; default renders transparent with `p.ink` text; disabled swaps in `p.panel` + `p.dim`. Primary variant currently uses `marginLeft: -1` for the edge-to-edge stroke join (H1 web-safe polish still pending). Callers: `MinEventSheet`, `MinRatingSheet`.
- `MinSheetRow({ label, value })` — sheet key/value row with bordered separator used by both minimal sheets.
...
### Rating flow
Unchanged from cloud-sync Phase 7 — local service writes fan out through `SyncingRatingRepository` to `scheduler.notifyLocalChange()`; `AppState` active triggers `scheduler.pullNow()`; merges persist via `LocalRatingRepository` and propagate to UI through `subscribeToRatingChanges` → `useRatings`. **Minimal-theme `MinRatingSheet` edit flow** (B3): while the sheet is open for a given `event?.id`, scheduler-driven refreshes can change the `existing` prop without resetting the user's in-progress `efficiency` / `mood` / `reflection` edits — reseeding is gated on `[open, event?.id]` transitions only.
...
- **Minimal P2 Fix Round — Phase 3 (B3) Rating Sheet Edit Stability**: `MinRatingSheet.tsx` seed effect dependency array was pruned from `[open, event?.id, existing?.efficiency, existing?.mood, existing?.reflection]` (pre-fix) down to `[open, event?.id]`, so a ratings-list refresh that re-reads `existing` for the same open event no longer clobbers mid-edit local `efficiency` / `mood` / `reflection` state. Reseeding still happens on the open→close→open transition and on event-id changes, matching the design HTML behavior. Added `MinRatingSheet.test.tsx` with two assertions: (1) local draft survives a refreshed `existing` prop for the same event, (2) switching to a different event id reseeds from the new `existing`. Efficiency / mood / reflection inputs gained stable testIDs (`min-rating-efficiency-{1..5}`, `min-rating-mood-{1..5}`, `min-rating-reflection`) to support the test.
...
- `npm test` stays green; minimal-specific tests live in `src/themes/minimal/components/MinSettings.test.tsx` and `src/themes/minimal/components/MinRatingSheet.test.tsx`.
