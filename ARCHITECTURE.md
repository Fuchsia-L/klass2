...
- `components/StarlightRoot.tsx` — Container. Wires `useTheme`/`useThemeSettings`/`useEvents`/`useSemesterConfig`/`useRatings`/`useTodos` into palette-driven Starlight screens + modal sheets. Holds tab/event-sheet/rating-sheet/todo-sheet/week-offset/nudge-dismissal state. Internal `Screen` wrapper cross-fades screens with RN `Animated.timing`. Module-local helpers `withState(events, nowMs)`, `latestRatingsByEventId(ratings)`, `formatTime`, `todoDueLabel`, `nudgeCopy`, `toTimelineEvent`, `routeToTab`, `sameDay`.
...
- Internal screens `StarlightHome`/`StarlightMatrix`/`StarlightTodos`/`StarlightSettings` and modal frames `StarlightEventSheet`/`StarlightRatingSheet`/`StarlightTodoSheet` are Phase-3 placeholders built on Starlight primitives (background, NowLine, TimelineRow, TodoRow, SheetBtn, SheetRow, PaletteCell). They accept palette colors + plain data props — no feature-hook imports.
- Add-button dispatch (`StarlightTabBar.onAdd`): opens `StarlightTodoSheet` in create mode when `tab==='todos'`, otherwise opens `StarlightEventSheet` in create mode (`eventSheetId='new'`).
...
AsyncStorage → `useEvents` + `useTodos` + `useRatings` + `useSemesterConfig` → `StarlightRoot` → `expandRepeatingEvents(events, currentWeekStart..currentWeekEnd)` for Today (filtered by `sameDay(todayStart, tomorrowStart)`) and `expandRepeatingEvents(events, viewingWeekStart..viewingWeekEnd)` for Matrix → `withState(_, Date.now())` → `toTimelineEvent` → `StarlightTimelineRow`. Ratings pass through `latestRatingsByEventId(ratings)`; past-day unrated events drive the nudge (`dismissedNudgeId` gate). Todo toggles call `toggleTodoComplete(id)` directly from the row. Palette changes in Settings call `useThemeSettings().setThemeName(palette.id)`.
