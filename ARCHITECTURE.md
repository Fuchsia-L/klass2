...
- `components/StarlightRoot.tsx` — Container. Wires `useTheme`/`useThemeSettings`/`useEvents`/`useSemesterConfig`/`useRatings`/`useTodos` into palette-driven Starlight screens + modal sheets. Delegates Today/Week rendering to `StarHome`/`StarMatrix`; holds placeholder `StarlightTodos`/`StarlightSettings` + `StarlightEventSheet`/`StarlightRatingSheet`/`StarlightTodoSheet`. Holds tab/event-sheet/rating-sheet/todo-sheet/week-offset/nudge-dismissal state. Internal `Screen` wrapper cross-fades screens with RN `Animated.timing`. Module-local helpers `withState(events, nowMs)`, `latestRatingsByEventId(ratings)`, `formatTime`, `todoDueLabel`, `nudgeCopy`, `routeToTab`, `sameDay`.
...
- Today/Week rendering is delegated to `StarHome`/`StarMatrix` (see below). `StarlightTodos`/`StarlightSettings` and modal frames `StarlightEventSheet`/`StarlightRatingSheet`/`StarlightTodoSheet` remain Phase-3 placeholders built on Starlight primitives (TodoRow, SheetBtn, SheetRow, PaletteCell). All children accept palette colors + plain data props — no feature-hook imports.
